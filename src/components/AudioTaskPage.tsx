...
export function AudioTaskPage() {
  const [request, setRequest] = useState<AudioProcessRequest>({
    file: null,
    typeTask: 'summary',
    userId: '',
    callbackUrl: '',
  ...
  const handleClear = useCallback(() => {
    runIdRef.current += 1;
    setRequest({ file: null, typeTask: 'summary', userId: '', callbackUrl: '' });
    setSteps(buildInitialSteps());
    setExecution({ status: 'idle', response: null });
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[1440px] flex-col gap-2 px-4 py-4">
        ...
        <main className="grid flex-1 gap-2 md:grid-cols-[minmax(320px,0.85fr)_minmax(0,1.15fr)] xl:grid-cols-[minmax(320px,520px)_minmax(0,1fr)]">
          <FormPanel
            ...
            onTaskTypeChange={(typeTask) => setRequest((current) => ({ ...current, typeTask }))}
            onUserIdChange={handleTextField('userId')}
            onCallbackChange={handleTextField('callbackUrl')}
            onClear={handleClear}
            onRun={handleRun}
